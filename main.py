...
async def on_raw_reaction_add(payload):
    ...
    if payload.emoji.name == "📩":
        ticketCategory = discord.utils.get(guild.categories, id=810910121236234310)
        if payload.channel_id == 842704847966175252:
            userMention = f'<@{payload.member.id}>'
            requestSupportChannel = client.get_channel(842704847966175252)
            requestSupportMessage = await requestSupportChannel.fetch_message(842704862247518229) #Put id in
            ...
            embed.set_footer(text='EU Rust Tournaments', icon_url="https://media.discordapp.net/attachments/798922297062850600/800336476826763334/chingyhasasmallcock.gif")
